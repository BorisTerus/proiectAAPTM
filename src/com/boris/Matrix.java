...
public class Matrix {
    ...
    public Matrix(double[][] data) {
        ...
        IntStream.range(0, this.data.length).forEach(
                row -> IntStream.range(0, this.data[0].length).forEach(
                        column -> this.data[row][column] = data[row][column]
                )
        );
    }
    ...
    public Matrix scalarOperation(double x, ScalarOperation scalarOperation) {
        ...
        IntStream.range(0, data.length).forEach(row ->
                IntStream.range(0, data[0].length).forEach(
                        column -> {
    ...
    public Matrix add(Matrix matrix) throws Exception {
        if ((data.length != matrix.data.length) ||
            ...
            throw new Exception("matrices must have matchin size");
        }
        double returnData[][] = new double[data.length][data[0].length];
        IntStream.range(0, data.length).forEach(row ->
    ...
    public Matrix transpose() {
        ...
        IntStream.range(0, data.length).forEach(row ->
                IntStream.range(0, data[0].length).forEach(
                        column -> returnData[column][row] = data[row][column]
                ));
        return new Matrix(returnData);
    }
    ...
    public Matrix exp() {
        IntStream.range(0, data.length).forEach(r ->
                IntStream.range(0, data[0].length).forEach(c ->
                        data[r][c] = Math.exp(data[r][c])));
        return this;
    }
        ...
        IntStream.range(0, data.length).forEach(row -> {
            ...
            IntStream.range(0, data[0].length).forEach(column -> {
                ...
                if (data[row][column] >= 0) bodySB.append(" " + String.format("%.2f", data[row][column]));
                    ...
                    bodySB.append(" " + String.format("%.2f", data[row][column]));
